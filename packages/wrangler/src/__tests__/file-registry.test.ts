...
describe("FilesystemWorkerRegistry", () => {
	...
	it("should initialize an empty registry", async () => {
		...
		// Verify that the registry directory was created
		const registryDirPath = tempDir;
		expect(fs.existsSync(registryDirPath)).toBe(true);
	...
	it("should handle unregistering non-existent worker", async () => {
		...
		// Check that the logger.debug was called with the expected message
	...
	// TODO: This is a difficult problem, but could potentially be solved by maintaining a registry that is
